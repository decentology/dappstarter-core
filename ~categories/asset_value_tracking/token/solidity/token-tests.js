...
it(`can transfer funds between accounts using transfer()`, async function () {
    try {
        ...
        assert.equal(balanceNewSource.toString(10), balanceOldSource.sub(transferAmount.mul(unitMultiple)).toString(10), "Incorrect source new balance value");
        assert.equal(balanceNewTarget.toString(10), balanceOldTarget.add(transferAmount.mul(unitMultiple)).toString(10), "Incorrect target balance value");
    } catch (e) {
        assert.fail(e.message);
    }
});
...
it(`can approve funds transfer using approve() and check spend amount using allowance()`, async function () {
    try {
        ...
        let allowance  = (await DappLib.allowance({
            sourceAccount: sourceAccount,
            targetAccount: targetAccount
        })).result;

        assert.equal(allowance.toString(), transferAmount, "Incorrect transfer allowance value");
    }
    catch(e) {
        assert.fail(e.message);
    }
...
it(`can transfer allowance funds between accounts using transferFrom()`, async function () {
    try {
        var sourceAccount = config.users[0];
        var targetAccount = config.users[1];
        var transferAmount = new BN(500).mul(unitMultiple); // 500 x 10^3 = 500000

        let sourceOldBalance = (await DappLib.balanceOf({
            account: sourceAccount
        })).result;
        let targetOldBalance = (await DappLib.balanceOf({
            account: targetAccount
        })).result;
        let oldAllowance = (await DappLib.allowance({
            sourceAccount: sourceAccount, 
            targetAccount: targetAccount
        })).result;
        ...
        let sourceNewBalance = (await DappLib.balanceOf({
            account: sourceAccount
        })).result;
        let targetNewBalance = (await DappLib.balanceOf({
            account: targetAccount
        })).result;
        let newAllowance = (await DappLib.allowance({
            sourceAccount: sourceAccount, 
            targetAccount: targetAccount
        })).result;

        assert.equal(sourceNewBalance.toString(), sourceOldBalance.sub(transferAmount).toString(), "Incorrect source new balance value");
        assert.equal(targetNewBalance.toString(), targetOldBalance.add(transferAmount).toString(), "Incorrect target new balance value");
        assert.equal(newAllowance.toString(), oldAllowance.sub(transferAmount).toString(), "Incorrect new allowance value");    
    }
    catch(e) {
        assert.fail(e.message);
    }
...
it(`cannot transfer funds between accounts using transferFrom() unless authorized`, async function () {
    try {
        ...
        assert.equal(sourceNewBalance.toString(), sourceOldBalance.toString(), "Incorrect source new balance value");
        assert.equal(targetNewBalance.toString(), targetOldBalance.toString(), "Incorrect target new balance value");
