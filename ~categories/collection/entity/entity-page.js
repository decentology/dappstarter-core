...
export default class EntityPage extends LitElement {
  ...
  showPageNumbers() {
    ...
    if (parseInt(this.entitiesCount) % this.pagesInfo.resultsPerPage >= 1) {
      ...
      let pageNumbers = Math.ceil(
        parseInt(this.entitiesCount) / this.pagesInfo.resultsPerPage
      );
    ...
    return (
      '<div><label for="page">Page Number:</label><select id="page" data-field="page">' +
      optionsString +
      "</select></div>"
    );
  }
  ...
  showResultsPerPage() {
    return '<div><label for="results">Results Per Page:</label><select id="results" data-field="resultsPerPage"><option value="10">10</option><option value="20">20</option><option value="30">30</option><option value="40">40</option> </select></div>';
  }
    ...
    let content = html`
      ...
      >
        ...
        <action-card
          ...
          action="setEntity"
          method="post"
          fields="textInfo numericInfo"
        >
          <text-widget
            field="textInfo"
            label="Text"
            placeholder="Sample text value for entity"
          >
          </text-widget>
          ...
          <number-widget
            field="numericInfo"
            label="Number"
            placeholder="Sample numeric value for entity"
          >
          </number-widget>
        </action-card>
        ...
        <action-card
          ...
          action="setEntity"
          method="post"
          fields="textInfo numericInfo id"
        >
          <number-widget field="id" label="Entity ID" placeholder="Entity ID">
          </number-widget>
