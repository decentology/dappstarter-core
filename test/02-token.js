...
contract('Dapp Contract Tests', async (accounts) => {
    ...
    it(`can approve funds transfer using approve() and check spend amount using allowance()`, async function () {
      ...
      it(`can transfer allowance funds between accounts using transferFrom()`, async function () {
        ...
        var sourceAccount = config.users[0];
        var allowAccount = config.users[1];
        ...
        let oldAllowance = await config.dappStateContract.allowance.call(sourceAccount, allowAccount);
        
        await config.dappStateContract.transferFrom(sourceAccount, targetAccount, transferAmount, { from: allowAccount }); 
        ...
        let newAllowance = await config.dappStateContract.allowance.call(sourceAccount, allowAccount);
